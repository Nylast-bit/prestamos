...
export function PrestamoFormDialog({
  ...
  const getScoreBadge = (score: number | null | undefined) => {
    const s = score ?? 500;
    if (s >= 850) return { label: `Excelente (${s})`, bg: 'bg-green-100 text-green-800 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800/60' };
    if (s >= 700) return { label: `Bueno (${s})`, bg: 'bg-blue-100 text-blue-800 border-blue-200 dark:bg-blue-900/40 dark:text-blue-300 dark:border-blue-800/60' };
    if (s >= 500) return { label: `Regular (${s})`, bg: 'bg-yellow-100 text-yellow-800 border-yellow-200 dark:bg-yellow-900/40 dark:text-yellow-300 dark:border-yellow-800/60' };
    if (s >= 300) return { label: `Bajo (${s})`, bg: 'bg-orange-100 text-orange-800 border-orange-200 dark:bg-orange-900/40 dark:text-orange-300 dark:border-orange-800/60' };
    return { label: `Crítico (${s})`, bg: 'bg-red-100 text-red-800 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800/60' };
  };
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <div className="flex items-center justify-start gap-2">
                ...
                {formData.IdCliente && (
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border shadow-sm tracking-wide ${getScoreBadge(clientScore).bg}`}>
