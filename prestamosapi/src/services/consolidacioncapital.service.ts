...
export const getAllConsolidacionesCapitalService = async (idEmpresa: number) => {
  ...
  if (lista && lista.length > 0) {
    const ids = lista.map((c: any) => c.IdConsolidacion);
    ...
    if (auditorias) {
      ...
      for (const a of auditorias) {
        if (!auditMap.has(a.IdConsolidacion)) {
          let meta: any = {};
          try { meta = JSON.parse(a.Descripcion); } catch (e) { meta = { observaciones: a.Descripcion }; }
...
export const auditarConsolidacionService = async (
  ...
  const { data: consolidacion, error: errC } = await supabase
    .from("ConsolidacionCapital")
  ...
  if (registros) {
    for (const r of registros) {
      const tipo = (r.TipoRegistro || "").toLowerCase().trim();
      if (tipo === "auditoria") continue; // Excluir otros registros de auditoría del cálculo
      const monto = Number(r.Monto || 0);
      if (tipo === "ingreso") {
  ...
  if (errInsert) {
    logger.error("Error insertando registro de auditoría:", errInsert.message);
...
export const getHistorialAuditoriaService = async (idConsolidacion: number, idEmpresa: number) => {
  const { data: consolidacion } = await supabase
    .from("ConsolidacionCapital")
    .select("IdConsolidacion")
    .eq("IdConsolidacion", idConsolidacion)
  ...
  if (error) {
    logger.error("Error en getHistorialAuditoriaService:", error.message);
    throw new Error(`Error obteniendo historial de auditoría: ${error.message}`);
  }

  const historial = (registros || []).map((r: any) => {
    let meta: any = {};
    try {
