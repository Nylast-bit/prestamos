...
exports.processFixedExpenses = void 0;
...
const gastoFijoRegistroService = __importStar(require("./gastofijoregistro.service")); // Asegúrate que esta ruta y archivo existan
// src/services/gastoFijoJob.service.ts
// --- FUNCIÓN AUXILIAR: Obtener Fechas de Consolidación ---
const getConsolidacionDates = async (id, idEmpresa) => {
...
const getActiveFixedExpenses = async (idConsolidacion, idEmpresa) => {
    ...
    const gastosPendientes = gastos.filter(gasto => {
        ...
        const registroEnConsolidacion = gasto.GastoFijoRegistro.some((reg) => reg.IdConsolidacion === idConsolidacion);
        // Si no hay registro en esta consolidación, está pendiente.
        return !registroEnConsolidacion;
    });
    logger_1.logger.info(`DEBUG: ${gastosPendientes.length} gastos fijos pendientes de registro.`);
    return gastosPendientes; // Dejamos 'as any' para evitar el error de tipado.
};
 ...
 */
const processFixedExpenses = async (idConsolidacion, idEmpresa) => {
    // 1. Obtener el rango de fechas de la Consolidación (y convertirlos a objetos Date)
    const { FechaInicio, FechaFin } = await getConsolidacionDates(idConsolidacion, idEmpresa);
    ...
    const gastosPendientes = await getActiveFixedExpenses(idConsolidacion, idEmpresa);
    ...
    for (const gasto of gastosPendientes) {
        ...
        if (shouldRegister) {
            ...
            try {
                ...
                }, idEmpresa); // Pasamos empresa aquí también
                ...
                await registroConsolidacionService.createRegistroConsolidacionService({
                    ...
                    Monto: gasto.Monto,
                }, idEmpresa); // Pasamos aquí también
                registrosExitosos++;
            }
            catch (error) {
                logger_1.logger.error(`Error procesando gasto ID ${gasto.IdGasto}: ${error.message}`);
            }
        }
    }
    logger_1.logger.info(`✨ ${registrosExitosos} gastos fijos programados para el período.`);
    return { success: true, count: registrosExitosos };
};
exports.processFixedExpenses = processFixedExpenses;
