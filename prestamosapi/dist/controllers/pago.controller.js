...
exports.deletePago = exports.updatePago = exports.getPagoById = exports.getAllPagos = exports.getProximaCuota = exports.createPago = void 0;
...
const createPago = async (req, res) => {
    try {
        ...
        const prestamo = await client_1.default.prestamo.findUnique({
            where: { IdPrestamo },
            select: {
                IdPrestamo: true,
                MontoPrestado: true,
        ...
        if (prestamo.TipoCalculo?.toLowerCase() === "amortizable") {
            ...
            const cuotas = calcularCuotasAmortizable(Number(prestamo.MontoPrestado), Number(prestamo.InteresPorcentaje), prestamo.CantidadCuotas);
            ...
            const interesPorCuota = Number(prestamo.MontoPrestado) *
        ...
        const resultado = await client_1.default.$transaction(async (tx) => {
            // Actualizar préstamo
            const prestamoActualizado = await tx.prestamo.update({
                where: { IdPrestamo },
                data: {
                    CapitalRestante: Number(prestamo.CapitalRestante) -
...
const calcularCuotasAmortizable = (monto, tasaInteres, numeroCuotas) => {
    const cuotas = [];
...
const getAllPagos = async (req, res) => {
    try {
        const lista = await client_1.default.pago.findMany({
...
const getPagoById = async (req, res) => {
    ...
    try {
        const pago = await client_1.default.pago.findUnique({
            where: { IdPago: id },
            include: { Prestamo: true, Volante: true },
        });
        if (!pago)
...
const updatePago = async (req, res) => {
    try {
        ...
        if (!IdPago)
            return res.status(400).json({ error: "IdPago es requerido" });
        ...
        let tablaPagos = JSON.parse(prestamo.TablaPagos);
        // 3. Encontrar la cuota que corresponde a este pago
        const cuotaIndex = tablaPagos.findIndex((c) => c.NumeroCuota === pago.NumeroCuota);
        if (cuotaIndex === -1)
...
const deletePago = async (req, res) => {
    const id = Number(req.params.id);
    try {
        await client_1.default.pago.delete({ where: { IdPago: id } });
