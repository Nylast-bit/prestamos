...
exports.createPrestamo = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const data = req.body;
    // 1. Crear préstamo
    const nuevoPrestamo = await client_1.default.prestamo.create({
        data: {
            ...data,
        },
    });
    ...
    const consolidacion = await client_1.default.consolidacionCapital.findFirst({
        where: {
            FechaInicio: { lte: hoy },
            FechaFin: { gte: hoy },
        },
    ...
    await client_1.default.registroConsolidacion.create({
        data: {
...
exports.updatePrestamo = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const data = req.body;
    const prestamo = await client_1.default.prestamo.update({
        where: { IdPrestamo: Number(id) },
        data,
    });
    res.json(prestamo);
...
const calcularTasaPorCuota = async (req, res) => {
    try {
        ...
        const calcularCuotaPorTasa = (tasa) => {
            switch (tipoCalculo.toLowerCase()) {
                case "capital+interes":
...
const validarCuotaDeseada = (monto, cuotaDeseada, numeroCuotas) => {
...
exports.validarCuotaDeseada = validarCuotaDeseada;
